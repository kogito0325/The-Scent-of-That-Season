...
public class Scene1Script : MonoBehaviour
{
    ...
    IEnumerator TextPractice()
    {
        ...
        yield return StartCoroutine(NormalChat(str1 + str2, "날씨가 쌀쌀해졌네..", 0));
        yield return StartCoroutine(NormalChat("유가현", "안녕, " + str1 + str2 + "! 오늘 날씨가 춥지?",2));
        yield return StartCoroutine(NormalChat("유가현", str2 + " 진짜 춥다. 그치?",2));
        yield return StartCoroutine(NormalChat("유가현", "(진짜 졸라 춥네)",2));
